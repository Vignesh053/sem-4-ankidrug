...
import { Card, Deck, StudyMode } from '../types';
...
export const StudyList: React.FC<Props> = ({ deck, cards, onBack }) => {
  ...
  const filteredCards = useMemo(() => {
    ...
    return cards.filter(c => 
      c.generic.toLowerCase().includes(s) || c.brand.toLowerCase().includes(s)
    );
  }, [cards, search]);
    ...
    <div className="flex flex-col h-screen bg-white dark:bg-gray-900 transition-colors duration-300">
      ...
      <div className="sticky top-0 z-20 bg-white dark:bg-gray-800 border-b dark:border-gray-700 shadow-sm p-4">
        <div className="flex items-center justify-between mb-4">
          ...
          <h2 className="text-lg font-bold text-gray-800 dark:text-white truncate max-w-xs">{deck.name} Study List</h2>
        ...
        <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
             ...
             <div className="flex bg-gray-100 dark:bg-gray-700 rounded-lg p-1">
                <button 
                    ...
                    className={`px-3 py-1.5 text-sm font-medium rounded-md transition ${mode === 'generic_visible' ? 'bg-white dark:bg-gray-600 shadow text-blue-600 dark:text-blue-300' : 'text-gray-500 dark:text-gray-400'}`}
                >
                    Gen → ???
                </button>
                <button 
                    onClick={() => setMode('brand_visible')}
                    className={`px-3 py-1.5 text-sm font-medium rounded-md transition ${mode === 'brand_visible' ? 'bg-white dark:bg-gray-600 shadow text-blue-600 dark:text-blue-300' : 'text-gray-500 dark:text-gray-400'}`}
                >
                    ??? ← Brand
                </button>
                <button 
                    onClick={() => setMode('both_visible')}
                    className={`px-3 py-1.5 text-sm font-medium rounded-md transition ${mode === 'both_visible' ? 'bg-white dark:bg-gray-600 shadow text-blue-600 dark:text-blue-300' : 'text-gray-500 dark:text-gray-400'}`}
                >
                    Show Both
                </button>
             </div>
      ...
      <div className="flex-1 overflow-auto">
        <table className="w-full text-left border-collapse">
            <thead className="bg-gray-50 dark:bg-gray-800 sticky top-0 shadow-sm z-10 text-xs uppercase tracking-wider text-gray-500 dark:text-gray-400">
                <tr>
                    <th className="p-4 w-1/2">Generic Name</th>
                    <th className="p-4 w-1/2">Brand Name</th>
                </tr>
            </thead>
